...
dfEukaryota = subset_taxa(Gooddata, Kingdom=="Eukaryota")
get_taxa_unique(dfEukaryota, "Phylum") # to see how many Subdivision, we have 21 Subdivision
get_taxa_unique(dfEukaryota, "Class") # to see how many genera, 42 different Class
get_taxa_unique(dfEukaryota, "Order") # to see how many genera, 73 different Order
get_taxa_unique(dfEukaryota, "Family") # to see how many families, 96 different families
get_taxa_unique(dfEukaryota, "Genus") # to see how many genera, 151 different genera
get_taxa_unique(dfEukaryota, "Species") # to see how many species, 156 different species/OTU are present
...
# summarise abundance by Phylum + Sample + BOTH metadata variables
ps2.Phylum.rel.df <- ps2.Phylum.rel.df[, .(Abundance = sum(Abundance)), 
                                                 by = .(OTU, Sample,Well,Samples)]
...
pdf("Phylum-barplot_all.pdf", 
    width = 10, 
    height = 12)
Phylum.bar <- ggplot(data=ps2.Phylum.rel.df, aes(x=Sample, y=Abundance*100, fill=OTU)) + 
...
top_30_Phylum <- head(Phylum_totals$Phylum,5)
...
Deg <- ggplot(plot_data, aes(x = PCoA1, y = PCoA2, color = as.factor(Samples))) +
  ...
  labs(
    ...
    title = "Eukaryotic community according to Samples",
    color = "Samples"
  ) +
  theme_bw() +
  theme(
...
p_volcano <- ggplot(volcano_data, aes(x = log2FoldChange, y = -log10(padj), color = Significatif, label = Label)) +
  ...
  labs(title = "DESeq2 : B (n=2) vs W (n=2)", x = "Log2 Fold Change (W vs B)", y = "-Log10(padj)") +
...
p_bar <- ggplot(bar_data, aes(x = reorder(Genus, log2FoldChange), y = log2FoldChange,
  ...
  coord_flip() +
  labs(title = paste("Top", top_n, "genres différentiels (B vs W)"),
...
top_counts <- norm_counts[taxa_sig, ]
# Ajouter les noms de genres comme labels de lignes
rownames(top_counts) <- bar_data$Genus  # ou utiliser tax_info
# Annoter les colonnes par condition
col_annot <- data.frame(Condition = sample_data(ps.taxa.filt)$Condition)
rownames(col_annot) <- colnames(top_counts)

pheatmap(top_counts, scale = "row", annotation_col = col_annot,
         main = "Top 10 differentially abundant genera",
         fontsize_row = 10, filename = "Heatmap_B_vs_W.pdf")

pheatmap(top_counts, scale = "row", annotation_col = col_annot,
         main = "Top 10 differentially abundant genera",
         fontsize_row = 10, filename = "Heatmap_B_vs_W.png")
